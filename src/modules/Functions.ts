...
export default {
    ...
    Init() {
        const initTime = Date.now(),
            _functionBtnTemplate = $(".function-btn-template"),
        ...
        Object.keys(this.List).forEach((categoryName) => {
            ...
            categoryFunctions.forEach(categoryFunction => {
                if (!categoryFunction.divider) {
                    ...
                    if (categoryFunction.disabled) {
                        functionBtn.addClass("disabled")
                        if (Client.settings.GetSettings("hide_disabled_functions") === true) functionBtn.addClass("hide")
                    }
        ...
        functionButtons.forEach((functionData, functionName) => {
            functionData.element.on("click", async () => {
                if (!this.List["Quick Actions"].find((qAction: CategoryFunction) => qAction.id === functionName) || functionData.func.quickActionsOverride) {
                    ...
                    await Request.new(functionName, Utils.CompressData(Editor.GetValue(), false), Client.settings.GetSettings(), Client.session).then(async res => {
                        if (res instanceof Response) {
                            const _headers = res.headers,
                                ...
                                _mstime = _headers.get("uglifier-time")
                            let _response_body = ""
        ...
        CustomEvents.AddEventListener(window, "hide_disabled_functions", (e) => {
            const value = Client.settings.GetSettings(e.type)

            functionButtons.forEach(btn => {
                if (btn.element.hasClass("disabled")) {
                    btn.element.toggleClass("hide", value)
                }
            })
        })

        console.log(`Loaded Functions. (took ${Date.now() - initTime}ms)`);
    },

    List: {
        ["Quick Actions"]: [
            { disabled: true, name: "Quick Uglify", id: "uglify", icon_id: "draw", tooltip: "Uglify your script with the default methods.", quickActionsOverride: true },
            { name: "Obfuscate", id: "obfuscate", icon_id: "draw", tooltip: "Obfuscates your script with many GoofyLuaUglifier functions and the IronBrew2 VM.", quickActionsOverride: true },
            ...
            { name: "Undo", id: "undo", icon_id: "undo", tooltip: "Undo the last change.", quickActionsOverride: true },
        ...
        ["Functions"]: [
            ...
            },
            {
                ...
                id: "numberstrings",
                icon_id: "tag",
                tooltip: `Encodes string constants into number tables.<br><br>Example:<br><code class="multiline">print("Hello")</code><br>&equals;<br><code class="multiline">print(function({{176,4576,832}, {161,3381,2100}, ...}))</code>`
            },
            {
                disabled: true,
                ...
                tooltip: "Turns strings into random non sense.<br><br>Example:<br><code>'Hello World'</code> &equals; <code>'?ƒ*/涮в+%癉ц飴'</code><br><br><br><b>EXPERIMENTAL</b> - Might break the script"
            },
            { divider: true },
            {
                name: "Numbers",
            ...
            },
            {
                ...
                id: "hexnumbers",
                icon_id: "looks_two",
                tooltip: "Transforms numbers into hex numbers.<br><br>Example: <code>100.123</code> &equals; <code>0x64.1f7ced916874</code><br><br>(Decimal numbers do not work for LuaU)"
            },
            { divider: true },
            {
                disabled: true,
            ...
            },
            {
                ...
                id: "tablelennumbers",
                icon_id: "table_chart",
                tooltip: "Transforms all numbers into tables, containing empty tables and using the length of <br>the base table. (does that make sense?) <br><br>Example:<br><code>5</code> &equals; <code>#{{},{},{},{},{}}</code><br><br><b>NOTE:</b> Might slow down the script!"
            },
            { divider: true },
            {
                name: "Table Lookup",
